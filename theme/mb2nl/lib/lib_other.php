...
function theme_mb2nl_course_fields( $courseid, $divs = true )
{
	...
	foreach ( $fields as $f )
	{
		$isvalue = isset( $f['configdata']['maxdate']) ? userdate( $f['value'], get_string( 'strftimedatemonthabbr', 'theme_mb2nl' ) ) : $f['value'];
		$islink = isset( $f['configdata']['link'] ) ? $f['configdata']['link'] : '';

		$output .= '<li class="fieldname-' . $f['shortname'] . '">';
		$output .= '<span class="name">'. $f['name'] . ':</span>';
		$output .= $islink ?  theme_mb2nl_course_fields_link($f['configdata'], $isvalue) : '<span class="value">' . $isvalue . ' </span>';
		$output .= '</li>';
	}
 ...
 *
 */
function theme_mb2nl_course_fields_link( $options, $text )
{

	$output = '';
	$url = str_replace('$$', $text, $options['link']);

	$output .= '<a class="value" href="' . $url . '" target="' . $options['linktarget'] . '">'. $text . '</a>';
...
function theme_mb2nl_get_course_fields( $courseid )
{
	...
	foreach ( $datas as $data )
	{
		...
		$fields[] = array(
			...
			'configdata' => $data->get_field()->get('configdata'),
