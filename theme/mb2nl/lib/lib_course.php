...
function theme_mb2nl_section_module_list( $sectionid, $link = false, $active = false )
{
	...
	foreach ( $modules as $k=>$m )
	{
		...
		$output .= '<span class="itemimage"><img class="activityicon" src="' . $m['icon'] . '" alt="' . $m['name'] . '"></span>';
 ...
 *
 */
function theme_mb2nl_get_course_sections( $ccourse = false, $sectionid = 0 )
{
	global $COURSE;
	...
	foreach ( $sections as $section )
	{
		if ( ! $section->visible || ! $section->sequence || ($sectionid > 0 && $sectionid != $section->id) )
		{
			continue;
		}
...
function theme_mb2nl_module_sections( $block = false )
{
	...
	if ( ! is_object( $PAGE->cm ) || ! theme_mb2nl_is_toc() )
	{
		return;
	}
	...
	foreach ( $sections as $section )
	{
		...
		$isactive = $PAGE->cm->section == $section['id'] ? ' active' : '';

		$output .= '<div class="coursetoc-section coursetoc-section-' . $section['num'] . $completecls . $isactive . '" data-id="' . $section['id'] . '">';
...
function theme_mb2nl_toc_class()
{
	...
	if ( $toc && $ismodule && ! $editing && ! $fullscreen )
	{
		return true;
	}
...
function theme_mb2nl_is_toc()
{
	...
	if ( $COURSE->id > 1 && theme_mb2nl_theme_setting($PAGE, 'coursetoc') && count( theme_mb2nl_get_course_sections() ) )
	{
		return true;
	}
...
function theme_mb2nl_activityheader()
{
	...
	if ( $headercontent['additional_items'] )
	{
